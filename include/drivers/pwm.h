...
struct pwm_driver_api {
	...
	void (*set_phase_voltages)(const struct device *dev, float ua, float ub, float uc);
 ...
 */
static inline void pwm_set_phase_voltages(const struct device *dev, float ua, float ub, float uc)
{
	//  float a, b, c;
	const struct pwm_driver_api *api = dev->api;
...
static inline void svpwm_enable_threephase_channle(const struct device *dev)
{
	const struct pwm_driver_api *api = dev->api;
	if (api->set_phase_state) {
		api->set_phase_state(dev, 1);
...
static inline void svpwm_disable_threephase_channle(const struct device *dev)
{
	const struct pwm_driver_api *api = dev->api;
	if (api->set_phase_state) {
		api->set_phase_state(dev, 0);
