...
#define MOTOR_DEBUG_IQ		     (0.04f)
#define MOTOR_DEBUG_SELFANGLE	     (0.002f)

/*
偏置角的获取:
		1、使用自动校准代码获取(107就是这样获取到的)
			[CONFIG_MOTOR_DEBUG_SELFLOOPMODE=y CONFIG_ABS_TYPE_ENCODER_CALIBRA=y]
			不同的MOTOR_DEBUG_IQ和MOTOR_DEBUG_SELFANGLE对结果影响很大，
			MOTOR_DEBUG_SELFANGLE尽可能被360整除，MOTOR_DEBUG_IQ大一点确保能尽快到达指定位置
		2、给定一个合适的MOTOR_DEBUG_IQ，使其处于零,手动转动电机，观察每个0点，然后读出偏置角(115.0f就是这样获取到的)
*/
// #define MOTOR_ABSENCODER_TYPE_OFFSET (115.0f - 90)
#define MOTOR_ABSENCODER_TYPE_OFFSET (107.0f - 90)
...
#define TOQREMODE_Q_KP	      (0.4f)
#define TOQREMODE_Q_KI	      (0.08f)
#define TOQREMODE_Q_KC	      (0.5f)
#define TOQREMODE_Q_MAX_LIMIT (12.0f)
#define TOQREMODE_Q_MIN_LIMIT (-TOQREMODE_Q_MAX_LIMIT)
