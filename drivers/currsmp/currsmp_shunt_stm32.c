...
static void currsmp_shunt_stm32_get_currents(const struct device *dev,
  ...
  data->adc_channl_a =
      LL_ADC_INJ_ReadConversionData12(cfg->adc, LL_ADC_INJ_RANK_1);
  data->adc_channl_b =
      LL_ADC_INJ_ReadConversionData12(cfg->adc, LL_ADC_INJ_RANK_2);
  data->adc_channl_c =
      LL_ADC_INJ_ReadConversionData12(cfg->adc, LL_ADC_INJ_RANK_3);

  data->adc_f_channl_a =
      moving_avg_update(data->cha_filter, data->adc_channl_a); // TODO
  data->adc_f_channl_b =
      moving_avg_update(data->chb_filter, data->adc_channl_b); // TODO
  data->adc_f_channl_c =
      moving_avg_update(data->chc_filter, data->adc_channl_c); // TODO
  //计算偏置
  data->adc_average = moving_avg_update(
      data->average_filter,
      (data->adc_channl_a + data->adc_channl_b + data->adc_channl_c) / 3);
  // 2. 改进转换公式
...
static void currsmp_shunt_stm32_get_bus_vol_curr(const struct device *dev,
                                                 float *bus_vol,
                                                 float *bus_curr) {
...
static int currsmp_shunt_stm32_init(const struct device *dev) {
  ...
  if (!cfg->slave_mode_flag) {
    ...
    LL_ADC_CommonInit(__LL_ADC_COMMON_INSTANCE(cfg->adc),
  ...
  if (!cfg->slave_mode_flag) {
    LL_ADC_REG_SetSequencerRanks(cfg->adc, LL_ADC_REG_RANK_1,
                                 LL_ADC_CHANNEL_15);
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_15,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_15,
                                LL_ADC_SINGLE_ENDED);
    LL_ADC_SetChannelPreselection(cfg->adc, LL_ADC_CHANNEL_15);

    LL_ADC_INJ_SetSequencerRanks(cfg->adc, LL_ADC_INJ_RANK_1,
                                 LL_ADC_CHANNEL_14);
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_14,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_14,
                                LL_ADC_SINGLE_ENDED);

    LL_ADC_INJ_SetSequencerRanks(cfg->adc, LL_ADC_INJ_RANK_2,
                                 LL_ADC_CHANNEL_16);
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_16,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_16,
                                LL_ADC_SINGLE_ENDED);

    LL_ADC_INJ_SetSequencerRanks(cfg->adc, LL_ADC_INJ_RANK_3,
                                 LL_ADC_CHANNEL_17);
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_17,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_17,
                                LL_ADC_SINGLE_ENDED);
    ...
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_4,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_4,
                                LL_ADC_SINGLE_ENDED);
    ...
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_9,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_9,
                                LL_ADC_SINGLE_ENDED);
    ...
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_5,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_5,
                                LL_ADC_SINGLE_ENDED);

    LL_ADC_INJ_SetSequencerRanks(cfg->adc, LL_ADC_INJ_RANK_2, LL_ADC_CHANNEL_8);
    LL_ADC_SetChannelSamplingTime(cfg->adc, LL_ADC_CHANNEL_8,
                                  LL_ADC_SAMPLINGTIME_1CYCLE_5);
    LL_ADC_SetChannelSingleDiff(cfg->adc, LL_ADC_CHANNEL_8,
                                LL_ADC_SINGLE_ENDED);
...
#define GENERATE_ISR_CODE(n)                                                   \
  ...
  static void UTIL_CAT(ISR_FUNC(n), _init)(void) {                             \
    IRQ_CONNECT(DT_IRQN(DT_INST_PARENT(n)),                                    \
                DT_IRQ(DT_INST_PARENT(n), priority), ISR_FUNC(n), NULL, 0);    \
...
#define CURRSMP_SHUNT_STM32_CURRSMP_SHUNT_INIT(n)                              \
  ...
  static const struct currsmp_shunt_stm32_config                               \
  ...
  static moving_avg_t filter_cha_##n = {.buffer = (buffer_cha_##n),            \
                                        .size = 32};                           \
  static int16_t buffer_chb_##n[32];                                           \
  static moving_avg_t filter_chb_##n = {.buffer = (buffer_chb_##n),            \
                                        .size = 32};                           \
  static int16_t buffer_chc_##n[32];                                           \
  static moving_avg_t filter_chc_##n = {.buffer = (buffer_chc_##n),            \
                                        .size = 32};                           \
  static int16_t buffer_average_##n[32];                                       \
  static moving_avg_t filter_average_##n = {.buffer = (buffer_average_##n),    \
                                            .size = 32};                       \
  ...
  DEVICE_DT_INST_DEFINE(                                                       \
      n, &currsmp_shunt_stm32_init, NULL, &currsmp_shunt_stm32_data_##n,       \
